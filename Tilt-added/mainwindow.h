...
private:
    ...
    double m_camWN   = 20.0;                 // natural frequency (rad/s) → 6–12 is good
    double m_camZeta = 0.98;                 // damping (0.7–1.0)
...
private:
    ...
    float m_slope = 0.0f;
    const int   m_step = 20;
    float       m_difficulty = 0.005; // 0.005f;
    const float m_difficultyIncrement = 0.0001; // 0.0001f;
    const float m_irregularity = 0.02; // 0.02f;
    ...
    // Pixelation controls
    static constexpr int PIXEL_SIZE = 6;   // size of one visual “pixel” in screen pixels
