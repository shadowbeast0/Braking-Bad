...
void CarBody::simulate(const QVector<Line>& terrain, bool accelerating, bool braking) {
    ...
    for (const Line& line : terrain) {
        ...
        for (const Point& point : hitbox) {
            ...
            if (dist <= 4 && intersectionx >= line.getX1() - 1 && intersectionx <= line.getX2() + 1) {
                ...
                vNormalToLine = vNormalToLine * Constants::RESTITUTION / (1 + std::exp(-vNormalToLine));
    ...
    for (int i = 0; m_isAlive && i < m_wheels.size(); i++) {
        ...
        double theta = -std::atan2(m_cy - wheel->getY(), m_cx - wheel->getX());
        if (accelerating) {
            wheel->updateR(Constants::ROTATION * std::sin(theta), Constants::ROTATION * std::cos(theta));
        }
        if (braking) {
            wheel->updateR(-Constants::ROTATION * std::sin(theta), -Constants::ROTATION * std::cos(theta));
        }
