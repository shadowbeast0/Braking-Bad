...
void IntroScreen::paintEvent(QPaintEvent*) {
    ...
    // TITLE
    const QString title = "F1: THE GAME";
...
void IntroScreen::drawPixelText(QPainter& p, const QString& s, int gx, int gy, int scale, const QColor& c, bool bold)
{
    ...
    for (int i = 0; i < s.size(); ++i) {
        ...
        // horizontal offset for this character with extra spacing
                ...
                if (row & (1<<(4-rx))) {
                    for (int sy=0; sy<scale; ++sy) {
                        for (int sx=0; sx<scale; ++sx) {
                            ...
                            if (bold) {
                                plot(baseOff + rx*scale+sx-1, ry*scale+sy,
                                     QColor(20,20,22));
                                plot(baseOff + rx*scale+sx+1, ry*scale+sy,
                                     QColor(20,20,22));
                                plot(baseOff + rx*scale+sx,   ry*scale+sy-1,
                                     QColor(20,20,22));
                                plot(baseOff + rx*scale+sx,   ry*scale+sy+1,
                                     QColor(20,20,22));
                            }
