...
public class Wheel {
    ...
    public double impulseX = 0, impulseY = 0;
    ...
    public void simulate(ArrayList<Line> lines, boolean accelerating, boolean braking){
        ...
        for(Line line : lines){
            ...
            if(dist < radius && intersectionx >= line.getX1() && intersectionx <= line.getX2()){
                ...
                // tangential and normal velocity before collision
                double vAlongLine0 = vx * Math.cos(theta) + vy * Math.sin(theta);
                double vNormalToLine0 = vy * Math.cos(theta) - vx * Math.sin(theta);

                double vAlongLine1 = vAlongLine0 * friction;
                double vNormalToLine1 = vNormalToLine0 * 1/(1+Math.pow(3, -vNormalToLine0));
                ...
                vx = vAlongLine1 * Math.cos(theta) - vNormalToLine1 * Math.sin(theta);
                vy = vAlongLine1 * Math.sin(theta) + vNormalToLine1 * Math.cos(theta);

                double dAlongLine = vAlongLine1 - vAlongLine0;
                double dNormaltoLine = vNormalToLine1 - vNormalToLine0;
                impulseX += dAlongLine * Math.cos(theta) - dNormaltoLine * Math.sin(theta);
                impulseY += dAlongLine * Math.sin(theta) + dNormaltoLine * Math.cos(theta);
